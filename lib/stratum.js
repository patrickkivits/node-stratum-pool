...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    this.simulateSocketFromWS = function(ws){
        //Socket emulation
        var socket = {}
            ...
            if (!port_options.websocket) {
                ...
                if (port_options.ssl) {
                    app = require('https').createServer({
                        key:  fs.readFileSync( port_options.ssl_key ),
                        cert: fs.readFileSync( port_options.ssl_cert )
                    }, processRequest );
                } else {
                    app = require('http').createServer( processRequest );
                }
                app.listen( parseInt(port), '0.0.0.0', serverStartedCallback );

                wss = new WebSocketServer({ server: app });
                wss.on('connection', function(ws) {
                    _this.handleNewClient(_this.simulateSocketFromWS(ws));
                });
            }
        });
    })();
